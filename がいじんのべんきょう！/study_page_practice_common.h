...
namespace study::practice { //TODO(fran): it's not yet clear if this should go inside a practice::common namespace or smth else
	...
	auto prepare_practice(ProcState* state) {
		struct practice_data { page_type page; void* data; } res;
		...
		case available_practices::writing:
		{
			...
			for (i32 tries = 1; tries <= max_retries; tries++) {
				...
				res.page = page_type::practice_writing;
				res.data = data;
				break;
			}
		} break;
		case available_practices::multiplechoice:
		{
			...
			res.page = page_type::practice_multiplechoice;
			res.data = data;
		} break;
		case available_practices::drawing:
		{
			...
			for (i32 tries = 1; tries <= max_retries; tries++) {
				...
				data->question_str = str_for(&data->question, data->question_type).str;
				res.page = page_type::practice_drawing;
				res.data = data;
				break;
			}
		} break;
	...
	void _next_practice_level(HWND hwnd, UINT /*msg*/, UINT_PTR anim_id, DWORD /*sys_elapsed*/) {
		...
		if (state->practice_cnt-- > 0) {
			...
			preload_page(state, page_type::review_practice, &state->multipagestate.temp_practices);
			set_current_page(state, page_type::review_practice);
		}
	}
