...
//BIG TODO(fran): we can actually pre allocate the sizes for everything, and assign structures to sections of the code for reuse, going back to the handmade hero mentallity that'd help a lot, no need for mallocs all over the place, at most we should implement re allocation, but even that's not necessary if we fixed every size, I was thinking about differentiating current size and allocated size for things like strings, but that's also not needed, with fixed sizes we can have everything allocated on the correct size from the start, the content on the pages of this program is very much fixed and controllable
//BIG TODO(fran): once we get this worker make a resizer, that'd save lots of problems
...
//Unix time has always been signed! Reasons for it: apparently c didnt support unsigned numbers at the time, also been signed allows you to go before 1970, not only forwards, which at the time might have seemed important since they were close to "before 1970", finally that means that you have 31 bits to go forward which leaves you at 2038, when going 64 bits you get 63 to go forward which leaves you beyond what anybody reading this will ever see
