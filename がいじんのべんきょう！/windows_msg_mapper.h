...
const char* msgToString(unsigned int msg) {
	...
	if (res == NULL)
	{
		static char unkown[40] = "UnknownMessage ";
		static size_t len = strlen(unkown);
		sprintf_s(unkown + len, sizeof(unkown) / sizeof(unkown[0]) - len, "%u | 0x%08x", msg, msg);
		res = unkown;
	}
	return res;
}
...
static std::map<const char*, const char*, hkl_cmp_str> hklTranslation = {
{"0000041C","Albanian"},
{"00000401","Arabic (101)"},
{"00010401","Arabic (102)"},
...
//Mapping from HKL hex string to descriptive name
//Use GetKeyboardLayoutNameA to retrieve the necessary string to use this function
static const char* HKLtoString(char code[KL_NAMELENGTH])
{//thanks https://stackoverflow.com/questions/14701095/how-to-get-keyboard-layout-name-from-a-keyboard-layout-identifier
	//NOTE: LOWORD is lang identifier and HIWORD is device identifier (ej dvorak)
	const char* res = hklTranslation[code];
	if (!res) res = "UnknownHKL";
	return res;
}
...
const char* HKLtoString(char code[KL_NAMELENGTH] /*HKL hex string*/) {
	const char* res = "UnknownHKL";
	return res;
}
#endif