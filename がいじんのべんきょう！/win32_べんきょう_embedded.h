...
namespace embedded {
	...
	namespace show_word_disambiguation {
		...
		constexpr cstr wndclass[] = L"がいじんの_wndclass_embedded_showworddisambiguation";
		struct Theme {
			struct {
				brush_group txt, bk, border;
			} brushes;
		...
		struct ProcState {
			...
			union {
				using type = HWND;
				struct { type mnemonic, lexical_category, notes, example_sentence; };
				type all[4];
			private: void _() { static_assert(sizeof(all) == sizeof(*this), "Come update the array to the correct count!"); }
			} controls;
		};
		...
		void resize_controls(ProcState* state) {
			...
			rect_i32 mnemonic = grid[0][0];
			rect_i32 lexical_category = grid[0][1];
			rect_i32 notes = grid[1][0];
			rect_i32 example_sentence = grid[1][1];

			rect_i32 bottom_most_control = example_sentence;

			int used_h = bottom_most_control.bottom();// minus start_y which is always 0
			int y_offset = (h - used_h) / 2;//Vertically center the whole of the controls

			MyMoveWindow_offset(controls.mnemonic, mnemonic, FALSE);
			MyMoveWindow_offset(controls.lexical_category, lexical_category, FALSE);
			MyMoveWindow_offset(controls.notes, notes, FALSE);
			MyMoveWindow_offset(controls.example_sentence, example_sentence, FALSE);
		}
		void show_controls(ProcState* state, bool show) {
			for (auto& c : state->controls.all) ShowWindow(c, show ? SW_SHOW : SW_HIDE);
		}
		...
		void set_word(HWND wnd, learnt_word16* word) {
			...
			if (state && word) {
				...
				SendMessage(state->controls.mnemonic, WM_SETTEXT, 0, (LPARAM)word->attributes.mnemonic.str);
				//SendMessage(state->controls.lexical_category, WM_SETTEXT, 0, (LPARAM)word->attributes.lexical_category.str);
				int lex_categ;//TODO(fran): HUGE SUPER HACK (involving lexical_category and lang.h)
				try { lex_categ = std::stoi(word->attributes.lexical_category.str); }
				catch (...) { lex_categ = -1; }
				AWT(state->controls.lexical_category, 200 + lex_categ);
				SendMessage(state->controls.notes, WM_SETTEXT, 0, (LPARAM)word->attributes.notes.str);
				SendMessage(state->controls.example_sentence, WM_SETTEXT, 0, (LPARAM)word->attributes.example_sentence.str);
				//TODO(fran): ask_for_repaint(state) ?
			}
		}
		LRESULT CALLBACK Proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
			...
			case WM_CREATE: {
				...
				SetWindowSubclass(state->controls.mnemonic, edit_oneline::show_on_hover::Proc, 0, 125);
				SetWindowSubclass(state->controls.lexical_category, edit_oneline::show_on_hover::Proc, 0, 123);
				SetWindowSubclass(state->controls.notes, edit_oneline::show_on_hover::Proc, 0, 126);
				SetWindowSubclass(state->controls.example_sentence, edit_oneline::show_on_hover::Proc, 0, 127);
				show_controls(state, true);
